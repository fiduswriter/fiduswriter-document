        ...
        switch (node.type) {
            ...
            case "figure": {
                ...
                if (image) {
                    ...
                    const imageDBEntry = this.imageDB.db[image], filePathName = imageDBEntry.image, filename = filePathName.toString().split("/").pop();
            ...
            case "table":
                if (node.content?.length) {
                    ...
                    if (node.content.length > 1 &&
                        ...
                        columns = node.content[1].content[0].content.reduce((columns, node) => columns + (node.attrs?.colspan || 1), 0);
                    }
                    const aligned = node.attrs.width === "100" ? "left" : node.attrs.aligned;
                    if (aligned === "center") {
                        start += "\n\n\\begin{center}";

...
export class PrintExporter extends HTMLExporter {
    ...
    async init() {
        ...
        await Promise.all(styleSheets.map(async (sheet) => await this.loadStyle(sheet)));
        this.converter = new HTMLExporterConvert(this.docTitle, this.doc.settings, this.docContent, this.htmlExportTemplate, this.imageDB, this.bibDB, this.csl, styleSheets, {
            relativeUrls: false
        });

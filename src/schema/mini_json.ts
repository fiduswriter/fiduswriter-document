...
export function toMiniJSON(node: Node): Record<string, unknown> {
    ...
    for (const attr in node.attrs) {
        if (!deepEqual((node.type as any).attrs[attr].default, node.attrs[attr])) {
            ...
            ;(obj.attrs as Record<string, unknown>)[attr] = ["string", "number", "boolean"].includes(
...
function toMiniMarkJSON(mark: Mark): Record<string, unknown> {
...
export function toFullJSON(miniJson: Record<string, unknown>, schema: Schema): Record<string, unknown> {
    if (!miniJson.type) {
        miniJson.type = "doc"
    }

...
export class DOCXExporterRels {
    ...
    hyperLinkStyle: string | null

    constructor(xml: XmlZip, docName: string) {
        this.xml = xml
    ...
    findMaxRelId(): void {
        ...
        rels.forEach(rel => {
            const id = Number.parseInt(
                String(rel.getAttribute("Id")).replace(/\D/g, "")
            )
    ...
    addImageRel(imgFileName: string): number {
        const rels = this.relsXML!.query("Relationships")
        const rId = ++this.relIdCounter
    ...
    addFootnoteRel(): number {
        ...
        if (footnotesRel) {
            ...
            const fnRId = Number.parseInt(
                String(footnotesRel.getAttribute("Id")).replace(/\D/g, "")
            )
            return fnRId
        }
        const rels = this.relsXML!.query("Relationships")
    ...
    addNumberingRel(): number {
        ...
        if (numberingRel) {
            ...
            const nuRId = Number.parseInt(
                String(numberingRel.getAttribute("Id")).replace(/\D/g, "")
            )
            return nuRId
        }
        const rels = this.relsXML!.query("Relationships")
